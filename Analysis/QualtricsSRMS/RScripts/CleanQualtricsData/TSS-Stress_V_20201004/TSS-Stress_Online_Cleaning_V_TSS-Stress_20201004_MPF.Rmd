---
title: "TSS-Stress_Cleaning_V_TSS-Stress_20201004"
output:
  html_document:
    code_folding: hide
    theme: default
...
mice66if(length(data.SonaPay!=0))(
...
data.Eligibility=data.filter %>%
  dplyr::select(SubID,PLATFORM, starts_with("Elig_")) %>%
  dplyr::mutate_all(as.character) %>%
  replace(.=="Normal Vision",0)%>%
  replace(.=="Corrected Vision", 0)%>%
  replace(.=="Non Corrected Vision", 1)%>%
  replace(.=="No, but I am fluent", .5)%>%
  dplyr::mutate(Elig_1 = replace(Elig_1, which(Elig_1 == "Yes"), 0))%>%
  dplyr::mutate(Elig_1 = replace(Elig_1, which(Elig_1 == "No"), 1))%>%
  dplyr::mutate(Elig_2 = replace(Elig_2, which(Elig_2 == "Yes"), 1))%>%
  dplyr::mutate(Elig_2 = replace(Elig_2, which(Elig_2 == "No"), 0))%>%
  dplyr::mutate(Elig_4 = replace(Elig_4, which(Elig_4 == "Yes"), 0))%>%
  dplyr::mutate(Elig_4 = replace(Elig_4, which(Elig_4 == "No"), 1))%>%
  dplyr::mutate(Elig_6 = replace(Elig_6, which(Elig_6 == "Yes"), 1))%>%
  dplyr::mutate(Elig_6 = replace(Elig_6, which(Elig_6 == "No"), 0))%>%
  dplyr::mutate_at(vars(starts_with("Elig_")),as.numeric)%>% # turn into the data type into numeric
  replace(.=="Prefer not to respond",NA)

#rename Questions about eligibility
data.Eligibility<-data.Eligibility %>%
  dplyr::rename(Elig_ColorBlindness=Elig_1,
         Elig_Dyslexic=Elig_2,
         Elig_NormalVision=Elig_3,
         Elig_NativeEnglish=Elig_4,
         Elig_YearsInUS=Elig_5,
         Elig_Transfer=Elig_6)
...
data.Demographic=data.filter %>%
  dplyr::select(SubID,PLATFORM,Dem_1,Dem_2,Dem_3,Dem_4,Dem_5,Dem_6,Dem_7) 

data.Demographic<-data.Demographic %>%
   dplyr::rename(Dem_Age=Dem_1,
         Dem_Sex=Dem_2,
         Dem_Gender=Dem_3,
         Dem_GenderOther=Dem_4,
         Dem_Ethnicity=Dem_5,
         Dem_EthDesc=Dem_6,
         Dem_Hispanic=Dem_7)
```
...
data.DASS=data.filter %>%
  dplyr::select(SubID, PLATFORM,starts_with("DASS.D_")) %>%
...
data.COVID19IS=data.filter %>%
  dplyr::select(SubID,PLATFORM, starts_with("C19IS_")) %>%
  dplyr::mutate_all(as.character) %>%
  replace(.=="Prefer not to answer",NA)%>%
  replace(.=="No change", 0)%>%
